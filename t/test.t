...
my $parse_helper_basic   = sub { Courriel::MMS->parse(@_) };
my $parse_helper_factory = sub { $fact->parse(@_)         };

sub run_tests {
    my $parse_helper = shift;

    {
        ...
        my $email = $parse_helper->( text => $message );
    ...
    {
        my $email = $parse_helper->( text => scalar( slurp( 't/data/MymtsRu.eml' ) ) );
    ...
    
    {
        ...
        my $email = $parse_helper->( text => $c_email->as_string );
    ...
    
    {
        ...
        my $email = $parse_helper->( text => $mtext );
...
run_tests( $parse_helper_basic );
run_tests( $parse_helper_factory );
